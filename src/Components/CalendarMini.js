// src/Components/CalendarMini.js
...
const parts = (d) => new Intl.DateTimeFormat("fa-IR-u-ca-persian",{ year:"numeric", month:"numeric", day:"numeric", weekday:"long" }).formatToParts(d).reduce((o,p)=>(o[p.type]=p.value,o),{});
...
export default function CalendarMini({ value, onChange, holidaysFa = [], compact = false }) {
  ...
  return (
    <div className={`cal ${compact ? "cal--compact": ""}`} dir="rtl">
      ...
      <div className="cal__grid">
        ...
        {days.map((d,i)=>{
          ...
          return (
            <button
              key={i}
              className={"cal__d" + (isToday ? " is-today" : "") + (isSel ? " is-selected" : "") + (holiday ? " is-holiday" : "")}
