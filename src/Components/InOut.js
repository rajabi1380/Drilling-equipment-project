...
const computeTotalStock = (rows) =>
  Math.max(
    0,
    rows.reduce(
      (sum, r) => (r.type === "in" ? sum + 1 : r.type === "out" ? sum - 1 : sum),
      0
    )
  );
...
/* ورودی کاتالوگ را همیشه به آرایه‌ی اقلام تبدیل می‌کند */
function normalizeCatalog(provider) {
  try {
    // اگر همین الان آرایه است
    if (Array.isArray(provider)) return provider;

    // اگر تابع است: با/بی پارامتر کال می‌کنیم
    if (typeof provider === "function") {
      let data;
      try {
        // اگر تابع پارامتر می‌پذیرد (length>=1)، با مقدار "all" صدا بزن
        data = provider.length >= 1 ? provider("all") : provider();
      } catch {
        data = [];
      }
      if (Array.isArray(data)) return data;
      if (data && typeof data === "object") {
        // اگر آبجکتِ چندبخشی برگشت داده، آرایه‌های داخلی را جمع کن
        const arr = Object.values(data).flatMap((v) => (Array.isArray(v) ? v : []));
        return arr;
      }
      return [];
    }

    // اگر آبجکت است (نه تابع و نه آرایه)
    if (provider && typeof provider === "object") {
      const arr = Object.values(provider).flatMap((v) => (Array.isArray(v) ? v : []));
      return arr;
    }
...
export default function InOut() {
  ...
  const handleExportExcel = () => {
    ...
    if (active === "inout") {
      ...
      const headers = Object.keys(
        rows[0] || { "نام تجهیز": "", "کد تجهیز": "", "سایز": "", "موجودی هر قطعه": 0 }
      );
      ...
      <div className="io-card">
          ...
          <div className="io-filter__fields">
            ...
            <div className="f-item">
              ...
              <DatePicker
                ...
                onChange={(val) =>
                  setFilterForm((v) => ({ ...v, fromDate: val }))
                }
                calendar={persian}
                locale={persian_fa}
                format={faFmt}
                  ...
                  {pagedRows.length ? (
                    pagedRows.map((r) => (
                      <tr
                        key={r.id}
                        className={
                          r.type === "in" ? "row-in" : r.type === "out" ? "row-out" : ""
                        }
                      >
                        <td>{r.name}</td>
                        <td>{r.code}</td>
        ...
        {active === "rig" && (
          <>
            ...
            />
            <div className="muted">
              {filtered.filter((r) => r.type === "rig").length} درخواست دکل به دکل
            </div>
          </>
        )}
...
function InModal({ onClose, onSubmit, catalogProvider }) {
  ...
  const submit = () => {
    if (!hasError) onSubmit({ name, code, size, enterDateObj, status, fromWhere, note });
  };
    ...
    <>
      <div className="modal-backdrop" onClick={onClose}>
        <div className="modal" onClick={(e) => e.stopPropagation()} dir="rtl">
          ...
          <footer className="modal__ftr">
            ...
            <button className="btn success" onClick={submit} disabled={hasError}>
              ثبت ورود
            </button>
          </footer>
        </div>
      ...
      <ItemPickerModal
        ...
        onPick={(it) => {
          setName(it.name || "");
          setCode(it.code || "");
          const autoSize = Array.isArray(it.sizes)
            ? it.sizes[0] || ""
            : it.size || "";
...
function OutModal({ onClose, onSubmit, catalogProvider }) {
  const [touched] = useState(true);
  const [name, setName] = useState("");
  const [code, setCode] = useState("");
  const [size, setSize] = useState("");
      ...
      <div className="modal-backdrop" onClick={onClose}>
        <div className="modal" onClick={(e) => e.stopPropagation()} dir="rtl">
          ...
          <footer className="modal__ftr">
            ...
            <button className="btn danger" onClick={submit} disabled={hasError}>
              ثبت خروج
            </button>
          </footer>
        </div>
...
function RigModal({ rigs, onClose, onSubmit, catalogProvider }) {
  const [touched] = useState(true);
  const [name, setName] = useState("");
  const [code, setCode] = useState("");
  const [size, setSize] = useState("");
      ...
      <div className="modal-backdrop" onClick={onClose}>
        <div className="modal" onClick={(e) => e.stopPropagation()} dir="rtl">
          ...
          <div className="form">
            ...
            <div className="row">
              ...
              <select
                className="input"
                value={rigFrom}
                onChange={(e) => setRigFrom(e.target.value)}
              >
            ...
            <div className="row">
              <select
                className="input"
                value={rigTo}
                onChange={(e) => setRigTo(e.target.value)}
              >
          ...
          <footer className="modal__ftr">
            ...
            <button className="btn warn" onClick={submit} disabled={hasError}>
              ثبت دکل به دکل
            </button>
          </footer>
        </div>
