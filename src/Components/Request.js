...
export default function Request() {
  // بوت از حافظه
  const boot = loadLS() || { open: [], archived: [], seq: 1 };
  ...
  useEffect(() => saveLS({ open: openOrders, archived: archivedOrders, seq }), [openOrders, archivedOrders, seq]);
  ...
  const onCreate = (payload) => {
    ...
    setOpenOrders(prev => [{ ...base, ...extra }, ...prev]);
    setShowModal(false);
    setSeq(newSeq);
        ...
        <form className="rq-filter" onSubmit={applyFilters}>
          <div className="grid">
            ...
            <div className="item">
              ...
              <input className="input" value={filterForm.unit} onChange={e=>setFilterForm(v=>({...v, unit:e.target.value}))} />
            </div>
            <div className="item">
              <label>شماره دستورکار</label>
              <input className="input" value={filterForm.wono} onChange={e=>setFilterForm(v=>({...v, wono:e.target.value}))} />
            </div>
        ...
        <section className="section">
          ...
          {showOpenTable && (
            <>
                  ...
                  <tbody>
                    {openSlice.length ? openSlice.map(r=>(
                      <tr key={r.id}>
                        ...
                        <td>
                          <button className="btn small" onClick={()=>completeAndArchive(r.id)}>تکمیل / بایگانی</button>
                        </td>
                      </tr>
                    )) : (
              ...
              {/* صفحه‌بندی بازها */}
              <div className="pagi">
                <button className="btn" disabled={pageOpen===1} onClick={()=>setPageOpen(1)}>« اول</button>
                <button className="btn" disabled={pageOpen===1} onClick={()=>setPageOpen(p=>p-1)}>‹ قبلی</button>
                <span className="muted">{pageOpen}/{totalOpenPages}</span>
                <button className="btn" disabled={pageOpen===totalOpenPages} onClick={()=>setPageOpen(p=>p+1)}>بعدی ›</button>
                <button className="btn" disabled={pageOpen===totalOpenPages} onClick={()=>setPageOpen(totalOpenPages)}>آخر »</button>
              </div>
        ...
        <section className="section">
          ...
          {showArcTable && (
            <>
              ...
              {/* صفحه‌بندی آرشیو */}
              <div className="pagi">
                <button className="btn" disabled={pageArc===1} onClick={()=>setPageArc(1)}>« اول</button>
                <button className="btn" disabled={pageArc===1} onClick={()=>setPageArc(p=>p-1)}>‹ قبلی</button>
                <span className="muted">{pageArc}/{totalArcPages}</span>
                <button className="btn" disabled={pageArc===totalArcPages} onClick={()=>setPageArc(p=>p+1)}>بعدی ›</button>
                <button className="btn" disabled={pageArc===totalArcPages} onClick={()=>setPageArc(totalArcPages)}>آخر »</button>
              </div>
          ...
          <div className="history-list">
            {history.map((r) => (
              <div key={r.id} className="history-card">
                ...
                {/* فعالیت‌ها: نام تعمیرکار، قطعه، تعداد */}
